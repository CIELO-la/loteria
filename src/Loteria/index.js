...
class Cantor {
  ...
  colorear = (texto) => {
    const txtHash = texto
      .split("")
      .reduce((hash, letra) => letra.charCodeAt(0) + ((hash << 5) - hash), 0);
    const color =
      "#" +
      [0, 1, 2]
        .map((i) =>
  ...
  seleccionarBaraja = (barajaId) => {
    if (!this.isHost) {
      console.log(
        `hostear un juego (Juego.asignarHost) para seleccionar baraja`
      );
      return;
    }
  ...
  leerCartaCantada = () =>
    this.cantadas === 0 ? {} : this.leerCarta(this.cartas[this.cantadas - 1]);
  ...
  verificar = () => {
    ...
    // si se marcaron
    const tablaValores = this.tabla.map(
      (carta) => carta[1] && indicesCantados.includes(carta[0])
    );

    // si ganó
    const esGanador = this.condiciones
      .reduce(
